...
void UBullCowCartridge::SetStartingWord()
{
    ...
    // Check minimum and max length
    for (int32 CollectionIndex = 0; CollectionIndex < ArraySize; ++CollectionIndex)
    {
        ListWord = *WordCollection[CollectionIndex];
        if (ListWord.Len() < 4 || ListWord.Len() > 10) //Magic numbers! eek - pull out if implementing difficulty options
        {
            WordCollection.RemoveAt(CollectionIndex);
            --ArraySize;
            --CollectionIndex;
